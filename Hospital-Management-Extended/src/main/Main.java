...
public class Main {
    private static final List<TreatmentHistory> treatmentHistories = new ArrayList<>();
    ...
    // Method to add a treatment history record
    private static void addTreatmentHistory() {
        ...
        treatmentHistories.add(th);
        System.out.println("Treatment added!");
    }

    // Method to view treatment history by patient ID
    private static void viewTreatmentHistory() {
        System.out.print("Enter Patient ID to search: ");
        String pid = scanner.nextLine();

        boolean found = false;
        for (TreatmentHistory th : treatmentHistories) {
            if (th.getPatientId().equalsIgnoreCase(pid)) {
